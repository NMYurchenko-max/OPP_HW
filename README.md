...
Вместо того, чтобы проверять, что список курсов студента или лектора содержит только один конкретный курс, условие сравнивнения изменено: весь список с одним курсом.

Для корректной работы функций, заменила следующую строку кода:
...
И в функции лекторов, аналогичную строку на:

``if course_name in lect.courses_attached:``

Также заменила return так, чтобы выводилось два знака после запятой:
...
### В результате полевых испытаний получены необходимые решения. Однако средняя оценка студентов по курсу возвращает 0

### не удается найти ошибку

### Есть техническая пробоема - настройки VSC не верны, исправить не могу ( за помощью обратилась, не оказана)

### Не сохраняет файлы, работаю одновременно в нескольких редакторах, VSC мне нравится больше
